...
// Modification by noriben - 2019
// Twitter: @noriben327
// Optimize for Unity. Looks like grass by Grabpass and Reflection.
 
 
 Shader "Noriben/RainDrops"
 {
     Properties
     {
         _Color("Color", Color) = (0.7, 0.8, 0.9, 1.0)
         _Scale("Scale", Range(0, 3)) = 1.0
		 _WidthScale("WidthScale", Range(0, 10)) = 1.0
         _Speed("Speed", Range(0, 10)) = 1.0
         _RainAmount("RainAmount", Range(0, 1)) = 0.5
     ...
     SubShader
     {
         ...
         Pass
         {
			...
			v2f vert (appdata v)
			{
             ...
             {
                float2 uv = float2(i.uv.x * 2.0 * _WidthScale, i.uv.y) * _Scale; //水滴のサイズ
 				float2 UV = i.uv.xy;
 				float3 M = float3(0.5, 0.5, 0.5);
 				float T = _Time.y + M.x * 2.0;
 
 				T = fmod(_Time.y, 102.);//水滴に付き方がランダムっぽくなる？
 				//T = lerp(T, M.x*102., M.z > 0. ? 1. : 0.);
 
 				float t = T * .2 * _Speed; //水滴のスピード
 
 				//float rainAmount = M.y;
                float rainAmount = 1.0 * _RainAmount; //雨の量
                ...
                //ハートの描画部分とズーム部分なので使わない
 				//story = S(0., 70., T);
                
                 
                //t = min(1., T / 70.);						// remap drop time so it goes slower when it freezes
                //t = 1. - t;
 				//t = (1. - t * t) * 70. * _Speed; //水滴の流れるスピード
                
                
 				//float zoom = 0.6;// lerp(.3, 1.2, story);		// slowly zoom out
 				//uv *= zoom;
                ...
                //float zoom = -cos(T * 0.2); //時間経過でズーミング
                float zoom = 1.0; //ズーム固定
                uv *= 0.7 + zoom * 0.3;
                ...
                float2 grabUV = (i.screenPos.xy / i.screenPos.w);
 				float4 texCoord = float4(grabUV.x + n.x, grabUV.y + n.y, 0, focus);
 				float4 lod = tex2D(_GrabTexture, texCoord);
                ...
                half4 refColor = UNITY_SAMPLE_TEXCUBE(unity_SpecCube0, reflDir);
 				...
 				float3 col = lod.rgb * _Color + (refColor.xyz * _Reflection); //ガラスの色、環境マップの強さ
                ...
                //エフェクト
 				...
 				//col = lerp(pow(col, float3(1.2, 1.2, 1.2)), col, heart); //heart
 				//fade *= S(102., 97., T);
